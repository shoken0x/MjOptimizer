    ...
    override func spec() {
        ...
        describe("includeAnkouOf"){
            it("works"){
                ...
                expect(pnl.includeAnkouOf(Pai.parse("m1t")!)).to.beTrue()
                expect(pnl.includeAnkouOf(Pai.parse("m2t")!)).to.beFalse()
            }
        }
        describe("includeShuntsuFrom"){
            it("works"){
                var pnl : PaiNumList = PaiNumList(paiList: Pai.parseList("m1tm1tm2tm3t")!)
                expect(pnl.includeShuntsuFrom(Pai.parse("m1t")!)).to.beTrue()
                expect(pnl.includeShuntsuFrom(Pai.parse("m2t")!)).to.beFalse()
            }
        }
        describe("removeAnkouOf"){
            ...
            it("returns nil"){
                var pnl : PaiNumList = PaiNumList(paiList: Pai.parseList("m1tm1tm2tm3t")!)
                expect(pnl.removeAnkouOf(Pai.parse("m1t")!) == nil).to.beTrue()
            }
        }
        describe("removeShuntsuFrom"){
            ...
            it("returns nil"){
                ...
                expect(pnl.removeShuntsuFrom(Pai.parse("m8t")!) == nil).to.beTrue()
            }
        }
    }
}