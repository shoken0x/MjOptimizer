...
class TotalSpec: QuickSpec {
    override func spec() {
        describe("parse"){
            ...
            it("ttp://dora12.com/2/yakuten/40mon.php"){
                var kyoku = Kyoku()
                kyoku.bakaze = Kaze.NAN
                kyoku.jikaze = Kaze.TON
                kyoku.honbaNum = 0
                ...
                kyoku.isParent = true
                kyoku.finishType = FinishType.NORMAL
                let paiStr = "m1tm2tm3tm8tm8tm8ts4ts5ts6tp8tp9tp9tp9tp7t"
                let mjpr : MjParseResult = MjParse.parse(paiStr, kyoku: kyoku)
                switch mjpr{
                case let .SUCCESS(agari):
                    expect(agari.fuNum).to(equal(40))
                    expect(agari.hanNum).to(equal(2))
                    expect(agari.score.total).to(equal(3900))
                    expect(agari.yakuNameList()).to(contain("doublereach"))
            ...
            it("ttp://dora12.com/2/yakuten/54mon.php"){
                var kyoku = Kyoku()
                kyoku.bakaze = Kaze.TON
                kyoku.jikaze = Kaze.TON
                kyoku.honbaNum = 0
                ...
                kyoku.isParent = true
                kyoku.finishType = FinishType.NORMAL
                let paiStr = "m3tm4tm5tm7tm8tm9ts7ts8ts9tp1tp3tp5tp5tp2t"
                let mjpr : MjParseResult = MjParse.parse(paiStr, kyoku: kyoku)
                switch mjpr{
                case let .SUCCESS(agari):
                    expect(agari.fuNum).to(equal(30))
                    expect(agari.hanNum).to(equal(3))
                    expect(agari.score.total).to(equal(6000))
                    expect(agari.yakuNameList()).to(contain("doublereach"))
                    expect(agari.yakuNameList()).to(contain("tsumo"))
